...
/**
  ...
  */
class SlackServiceSpec extends FlatSpec with DefaultFutures with DummyPullRequest {
  ...
  it should "post good, warning, danger pulls to incoming webhook" taggedAs SlackTest in {
    post { context =>
      ...
      pullRequest(createdAt = now.minusDays(1), title = "good", number = 2) ::
        pullRequest(createdAt = now.minusDays(slack.warningAfter.toDays), title = "warning", number = 3) ::
        pullRequest(createdAt = now.minusDays(slack.dangerAfter.toDays), title = "danger", number = 4) :: Nil
    }
  }
