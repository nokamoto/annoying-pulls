...
class IncomingWebhookService(context: StaticContext) {
  ...
  private[this] def exclude(pulls: List[PullRequest]) = {
    pulls.filterNot(_.issue.labels.exists(label =>
      github.excludedLabels.contains(label.name)))
  }

  private[this] def print(cond: Boolean, s: String): String =
    if (cond) s else ""
  ...
  private[this] def text(pulls: List[PullRequest]): String = {
    ...
    val hidden = print(excludedSize > slack.attachmentsLimit,
                       s"${excludedSize - slack.attachmentsLimit} hidden")
    val excluded =
      print(size > excludedSize, s"${size - excludedSize} excluded")
    val extended = hidden + print(hidden.nonEmpty && excluded.nonEmpty, ", ") + print(
      excluded.nonEmpty,
      excluded)
