...
class GithubService(context: Context)(implicit ec: ExecutionContext) extends Logger {
  ...
  private[this] def getPages[A : Reads](url: String): Future[List[A]] = {
    ...
    def req(next: String): Future[List[A]] = {
      ...
      val res = github.personalAccessToken.foldLeft(ws.url(next)) { case (req, token) =>
  ...
  private[this] def getRepos(ownerOpt: Option[Owner], path: Owner => String): Future[List[(Owner, Repo)]] = {
    ownerOpt match {
      case Some(owner) => getPages[List[Repo]](path(owner)).map(_.flatten).map(_.map((owner, _)))
  ...
  private[this] def getPullRequests(owner: Owner, repo: Repo): Future[List[PullRequest]] = {
    for {
      pulls <- getPages[List[Pulls]](s"${github.api}/repos/${owner.name}/${repo.name}/pulls").map(_.flatten)
